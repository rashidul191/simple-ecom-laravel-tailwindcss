    ...
    <form action="{{ route('admin.general-setting.store') }}" method="post" enctype="multipart/form-data">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4  w-full">
        <div class="flex w-full p-1">
          <img width="50" id="preMetaIcon" src="{{ business_setting('meta_icon') }}" alt="">
          <div class="w-full">
            <x-input-label class="!text-md" for="meta_icon" :value="__('Meta Icon')" />
            <x-text-input oninput="preMetaIcon.src = window.URL.createObjectURL(this.files[0])" id="meta_icon" class="block mt-1 w-full" type="file" name="meta_icon" :value="old('meta_icon')" />
            <x-input-error :messages="$errors->get('meta_icon')" class="mt-2" />
          </div>
        </div>
        <div class="flex w-full p-1">
          <img width="80" id="preLogo" src="{{ business_setting('logo') }}" alt="">
          <div class="w-full">
            <x-input-label class="!text-md" for="logo" :value="__('Logo')" />
            <x-text-input oninput="preLogo.src = window.URL.createObjectURL(this.files[0])" id="logo" class="block mt-1 w-full" type="file" name="logo" :value="old('logo')" />
